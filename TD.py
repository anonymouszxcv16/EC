...
class Agent(object):
    ...
    def init(self):
        ...
        self.conscious_replay = buffer.LAP(self.state_dim, self.action_dim, self.hp.zs_dim, self.device, self.args, self.args.buffer_size,
										   ...
										   prioritized="MC" in self.args.policy or "PER" in self.args.policy or "PSER" in self.args.policy or "RELO" in self.args.policy)
        self.unconscious_replay = buffer.LAP(self.state_dim, self.action_dim, self.hp.zs_dim, self.device, self.args, self.args.buffer_size,
                                             self.hp.batch_size, self.max_action, normalize_actions=True,
											 prioritized="MC" in self.args.policy or "PER" in self.args.policy or "PSER" in self.args.policy or "RELO" in self.args.policy)
    ...
    def compute_cql_loss(self, state, action):
        with torch.no_grad():
            zs = self.fixed_encoder_target.zs(state)
            actor, _ = self.actor_target(state, zs, deterministic=False)
            zsa_actor = self.fixed_encoder_target.zsa(zs, actor)
            zsa = self.fixed_encoder_target.zsa(zs, action)

            q_values = self.critic_target(state, actor, zsa_actor, zs).mean(1, keepdim=True)
    ...
    def estimate_loss(self, replay):
        ...
        if replay.size > 0:
            state, action, _, next_state, reward, _, _, not_done = replay.sample()
    ...
    def train(self):
        ...
        state, action, log_policy_old, next_state, reward, mc_score, zsa, not_done = self.conscious_replay.sample()

        # Update Encoder.
        if "SALE" in self.args.policy or "MC" in self.args.policy:
            with torch.no_grad():
            ...
            # Accumulate SALE loss
            self.sale_loss += encoder_loss.item()
        ...
        # Update Critic.
        with torch.no_grad():
            if not "REINFORCE" in self.args.policy:
        ...
        if "PER" in self.args.policy: # or ("MC" in self.args.policy and not "REINFORCE" in self.args.policy):
        ...
        elif "RELO" in self.args.policy or ("MC" in self.args.policy and not "REINFORCE" in self.args.policy):
            with torch.no_grad():
                ...
                # Accumulate ReLO losses
                self.relo_loss += loss.mean().item()
        ...
        # Update Actor.
        if self.training_steps % self.hp.policy_freq == 0 or "REINFORCE" in self.args.policy:
            actor, log_policy = self.actor(state, fixed_zs)

            if "REINFORCE" in self.args.policy:
                if self.conscious_replay.prioritized:
                    self.conscious_replay.update_priority((log_policy * mc_score).sum())

                if "PPO" in self.args.policy:
                    # ratio of probabilities
                    log_policy -= log_policy_old

                    # clip
                    clipped_log_ratio = torch.clamp(log_policy, torch.log(torch.tensor(1.0 - self.args.eps).to(self.args.device)),
                                                    torch.log(torch.tensor(1.0 + self.args.eps).to(self.args.device)))

                    gradient = torch.min(log_policy * mc_score, clipped_log_ratio * mc_score).sum()
                else:
                    gradient = (log_policy * mc_score).sum()

                self.loss_tot += gradient

                self.actor_optimizer.zero_grad()
                gradient.backward()
                self.actor_optimizer.step()

                self.conscious_replay.log_policy[self.conscious_replay.ind] = log_policy
            ...
            if self.args.offline == 1 and ("BCPO" in self.args.policy):
                Q_pi = Q
                q_pi = Q_pi.min(1, keepdim=True)[0]

                # advantage proxy relative to behavior action a (stop grad through q_beh to avoid instability)
                with torch.no_grad():
                    q_beh = Q_a.min(1, keepdim=True)[0]
                    adv = q_pi - q_beh

                    # advantage-weighted BC weight (clipped for stability)
                    w = torch.exp(-self.hp.alpha_bcpo * adv).clamp(max=10.0)

                # behavior cloning term (BCPO-style, optional)
                actor_loss = (w * (actor - action).pow(2).sum(-1)).mean()

            # BC
            elif self.args.offline == 1 and ("BC" in self.args.policy):
                # Accumulate BC loss
        ...
        if "MC" in self.args.policy:
            if self.t % self.args.interval == 0:
                # select topk experiences
                state, action, next_state, reward, r_correction, mc_correction, done = self.conscious_replay.select()

                self.unconscious_replay.add(state, action, next_state, reward, r_correction, mc_correction, 0, done)

            if self.t % self.args.interval == self.args.interval // 2 and self.unconscious_replay.size > 0:
                # fit experiences
                state, action, log_policy, next_state, reward, mc_score, zsa, done = self.unconscious_replay.fit()

                # back to conscious
                self.conscious_replay.add(state, action, next_state, reward, reward, mc_score, zsa, done, mc_score)
        ...
        if self.t % self.hp.target_update_rate == 0:
            ...
            if "RELO" in self.args.policy or "PER" in self.args.policy or "PSER" in self.args.policy or "MC" in self.args.policy:
